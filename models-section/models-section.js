...
(() => {
  ...
  const prevBtn = root.querySelector(".img-btn2 .left");
  const nextBtn = root.querySelector(".img-btn2 .right");
  ...
  function updateButtons() {
    // Left button: hide on first slide (index 0), show on others
    if (prevBtn) {
      prevBtn.style.visibility = index === 0 ? "hidden" : "visible";
    }

    // Right button: hide on last slide, show on others
    if (nextBtn) {
